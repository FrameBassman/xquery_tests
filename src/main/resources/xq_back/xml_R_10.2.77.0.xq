...
declare variable $propertyName := 'RemindCardNotScanned';

declare variable $defaultValue := 'true';

declare variable $previousValue := local:getPreviousValue($conn, 'SCO', $propertyName);
...
declare function local:getPreviousValue($conn as xs:integer, $moduleName as xs:string, $propertyName as xs:string) as xs:string? {
...
declare function local:deletePreviousValue($conn as xs:integer, $moduleName as xs:string, $propertyName as xs:string) {
...
if (exists($previousValue)) then (
  local:addPropertyWithValueFromDb($conn, $propertyName)
) else (
  local:addPropertyWithDefaultValue($propertyName, $defaultValue)
),
sql:close($conn)
